...
const byte TIPO_MODULO=0x21;
const byte NUMERO_FUNCIONES=0x05;
const byte FUNCIONES_MODULO[NUMERO_FUNCIONES]={SENSOR_HUMO,SENSOR_TEMPERATURA_HUMEDAD,SENSOR_MAGNETICO,SALIDAS_CONTROLADAS,SENSOR_MOVIMIENTO};
...
unsigned int tiempoReconexion=10000;
...
void setup(){
  //Configuracion de entradas/salidas a traves de los registro del ATmega328p
  DDRB=B11111011;
  DDRC=B00000111;
  DDRD=B00100010;
  PORTB|=B00011000;
  ...
  for(int i=0;i<NUMERO_FUNCIONES;i++){
    if(FUNCIONES_MODULO[i]==SENSOR_CAUDAL){
      TCCR1A=0;
      TCCR1B=0;
      TCCR1B|=(1<<CS12);
      TCNT1=3036;
      TIMSK1|=(1<<TOIE1);
      attachInterrupt(digitalPinToInterrupt(PIN_SENSOR_CAUDAL),interrupcionCaudalimetro,FALLING);
      interrupts();
    }
    if(FUNCIONES_MODULO[i]==SENSOR_CORRIENTE){
      sensorCorriente.current((int)PIN_SENSOR_CORRIENTE,55);
    }
    if(FUNCIONES_MODULO[i]==SENSOR_TEMPERATURA_HUMEDAD){
      sensorTempHum.begin();
    }
  }
...
//Interrupcion usada por el sensor caudalimetro
ISR(TIMER1_OVF_vect){
  TIMSK1 &= ~(1 << TOIE1);
  detachInterrupt(digitalPinToInterrupt(PIN_SENSOR_CAUDAL));
  numeroMililitrosInt+=((contadorPulsos/6.9)/60)*1000;;
  contadorPulsos=0;
  attachInterrupt(digitalPinToInterrupt(PIN_SENSOR_CAUDAL),interrupcionCaudalimetro,FALLING);
  TCNT1 = 3036;
  TIMSK1 |= (1 << TOIE1);
...
void obtenerDatosSensorCorriente(){
  double potenciaAparente=220.0*(sensorCorriente.calcIrms(1480));
  int potenciaAparenteCorregida=constrain(((int)potenciaAparente),0,22000);
...
//Subrutina de interrupcion para la lectura del caudalimetro
void interrupcionCaudalimetro(){
  contadorPulsos++;
}
